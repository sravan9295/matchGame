...
const initialState = {
  score: 0,
  secondsRemaining: 60,
  imageToBeMatched: imagesList[0],
  tabSelected: tabsList[0].tabId,
  isGameOver: false,
}
...
class MatchGame extends Component {
  ...
  onClickThumbnail(thumbnailId) {
    const {imageToBeMatched} = this.state
    const {id} = imageToBeMatched
    if (thumbnailId === id) {
      const randomNumber = Math.floor(Math.random() * imagesList.length)
      const randomImage = imagesList[randomNumber]
      this.setState(prevState => ({
        score: prevState.score + 1,
        imageToBeMatched: randomImage,
      }))
    } else {
      this.setState({isGameOver: true, secondsRemaining: 0})
  ...
  renderBody() {
    const {imageToBeMatched, tabSelected, isGameOver} = this.state
    const {imageUrl} = imageToBeMatched
    const thumbnailsList = imagesList.filter(
      item => item.category === tabSelected,
    )
      ...
      <div className="container">
        {isGameOver ? (
          ...
          <div className="body-content">
            <img className="image-to-be-matched" src={imageUrl} alt="match" />
            <div className="tabs-container">
              <ul className="tabs-ul-list">
  ...
  render() {
    const {score, secondsRemaining} = this.state

    return (
      <>
        <Navbar score={score} secondsRemaining={secondsRemaining} />
        {this.renderBody()}
      </>
    )
  }
